...
import numpy as np
#from utils.biformer1 import BiLevelRoutingAttention
from utils.mamba import SS2D
...
class lw_net(nn.Module):
    def __init__(self):
        super(lw_net, self).__init__()
        ...
        self.d_vmamba = nn.Sequential(SS2D(d_model=12, dropout=0.2, d_state=12))
        self.o_vmamba = nn.Sequential(SS2D(d_model=12, dropout=0.2, d_state=12))
    ...
    def forward(self, x):
        ...
        d_x_rgb = self.d_vmamba(d_x_rgb).permute(0,3,1,2)
        ...
        o_x_rgb = self.o_vmamba(o_x_rgb).permute(0,3,1,2)
